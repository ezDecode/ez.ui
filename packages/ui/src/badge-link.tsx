...
type BuiltInIcon = keyof typeof BUILT_IN_ICONS

function cx(...classes: (string | undefined | null | false)[]) {
  return classes.filter(Boolean).join(' ')
}

function getTooltipVars(seed: string): CSSProperties {
  let hash = 0
  for (let i = 0; i < seed.length; i += 1) {
    hash = (hash * 33 + seed.charCodeAt(i)) >>> 0
  }
  const rotate = (hash % 11) - 5
  const shiftX = ((hash >> 4) % 9) - 4
  return {
    '--tooltip-rotate': `${rotate}deg`,
    '--tooltip-shift': `${shiftX}px`,
  } as CSSProperties
}
...
export function BadgeLink({ href, children, icon, className }: BadgeLinkProps) {
  const baseClassName = cx(
    'group/badge relative inline-flex items-baseline gap-[0.2em] whitespace-nowrap align-baseline text-[1em] leading-[inherit] text-foreground-secondary',
  ...
  const tooltipVars = getTooltipVars(href)

  let iconNode: ReactNode = null
  if (typeof icon === 'string' && icon in BUILT_IN_ICONS) {
    const Icon = BUILT_IN_ICONS[icon as BuiltInIcon]
    iconNode = <Icon />
  } else if (icon) {
    iconNode = icon
  }
    ...
    <>
      {iconNode ? (
        <span
          ...
          style={tooltipVars}
        >
          <span className="tooltip-content bg-[oklch(31.4%_0_0/60%)]! backdrop-blur-md shadow-[inset_0_1px_0_oklch(100%_0_0/40%),inset_0_-1px_0_oklch(0%_0_0/42%),0_8px_22px_-10px_oklch(0%_0_0/75%),0_2px_8px_-4px_oklch(0%_0_0/55%)]! flex size-9 origin-bottom items-center justify-center rounded-xl text-foreground">
            <span className="inline-flex size-5 items-center justify-center text-foreground-secondary transition-colors duration-300 group-hover/badge:text-foreground group-focus-visible/badge:text-foreground [&_svg]:block [&_svg]:size-5 [&_svg]:shrink-0">
              {iconNode}
            </span>
          </span>
        </span>
